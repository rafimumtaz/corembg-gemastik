...
import { X, HeartHandshake, Plus, Loader2 } from 'lucide-react';
import { createRecipient, Recipient } from '@/lib/api';

interface RecipientModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (newRecipient: Recipient) => void;
}
...
export const RecipientModal: React.FC<RecipientModalProps> = ({
  ...
  const [type, setType] = useState<'PANTI' | 'PENERIMA'>('PANTI');
  const [address, setAddress] = useState('');
  const [capacity, setCapacity] = useState<number>(50);
  const [latitude, setLatitude] = useState<number>(-7.44);
  const [longitude, setLongitude] = useState<number>(112.72);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !address) {
      setError('Nama penerima dan alamat harus diisi.');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Gagal mendaftarkan penerima.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/10">
        ...
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-blue-50 to-cyan-50/50 px-6 py-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-slate-900">Registrasi Penerima (Panti/Posyandu)</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Nama Panti / Posyandu / Penerima</label>
            <input
              type="text"
              required
              value={name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Tipe Penerima</label>
              <select
                value={type}
                onChange={(e) => setType(e.target.value as 'PANTI' | 'PENERIMA')}
                className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              >
                <option value="PANTI">PANTI (Panti Asuhan/Posyandu)</option>
                <option value="PENERIMA">PENERIMA (Individu/Kelompok)</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Kapasitas Porsi</label>
              <input
                type="number"
                min="1"
                required
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>Menyimpan Penerima...</span>
                </>
              ) : (
                <>
                  <Plus className="h-4 w-4" />
                  <span>Daftarkan Penerima</span>
                </>
              )}
            </button>
          </div>
